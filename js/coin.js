...
(function () {
	...
	coinjs.bech32 = {'charset':'qpzry9x8gf2tvdw0s3jn54khce6mua7l', 'version':0, 'hrp':'mec', 'hrp_test':'tmec'}; //not supported

	coinjs.compressed = false;

	/* other vars */
	coinjs.developer = 'TODO'; // megacoin

	/* bit(coinb.in) api vars */
	coinjs.host = 'https://chainz.cryptoid.info/mec/api.dws?key=';
	coinjs.uid = '1';
...
//					var scr = script || u.getElementsByTagName("script")[0].childNodes[0].nodeValue;
					...
					if(segwit){
						...
						s.writeOp(0);
						s.writeBytes(coinjs.numToBytes(u.value*1, 8));
...
//					value += u.getElementsByTagName("value")[0].childNodes[0].nodeValue*1;
		...
		r.transactionHash = function(index, sigHashType) {
			...
			if((clone.ins) && clone.ins[index]){
				...
				var r = Crypto.util.bytesToHex(Crypto.SHA256(buffer, {asBytes: true}));
				return r;
			} else {
				return false;
			}
		...
		r.transactionHashSegWitV0 = function(index, sigHashType){
			...
			*/
			...
			var hashPrevouts = bufferTmp.length >= 1 ? Crypto.SHA256(bufferTmp, {asBytes: true}) : zero;
			...
			var hashSequence = bufferTmp.length >= 1 ? Crypto.SHA256(bufferTmp, {asBytes: true}) : zero;
			...
			if(sigHashType != 2 && sigHashType != 3){		// not sighash single & none
				...
				hashOutputs =Crypto.SHA256(bufferTmp, {asBytes: true});
			...
			var hash = Crypto.SHA256(buffer, {asBytes: true});
			return {'result':1,'hash':Crypto.util.bytesToHex(hash), 'response':'hash generated'};
		}
